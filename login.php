...
try {
    // Crear conexión usando los datos configurados
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($userInput) || empty($passInput)) {
            ...
            $query = $conn->prepare("
                ...
                FROM administrador 
                WHERE Usuario_Admin = :username AND Contrasena_Admin = :password
                UNION 
                SELECT 'user' AS user_type 
                FROM usuario 
                WHERE Usuario = :username AND Contrasena = :password
            ");
            $query->bindParam(":username", $userInput);
            $query->bindParam(":password", $passInput);
            $query->execute();
            ...
            if ($result) {
                ...
                if ($result['user_type'] === 'admin') {
                    header("Location: CEINTZ.html");
                } else {
                    header("Location: CEINTZusu.html");
                }
                exit();
            } else {
